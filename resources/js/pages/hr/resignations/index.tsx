...
export default function Resignations() {
  ...
  const ResignationDateField = ({ field, formData, handleChange }: any) => (
    <Input
      type="date"
      value={formData.resignation_date ? 
        (formData.resignation_date.includes('T') ? formData.resignation_date.split('T')[0] : formData.resignation_date) 
        : ''}
      onChange={(e) => {
        const newDate = e.target.value;
        handleChange('resignation_date', newDate);
        if (newDate) {
          const date = new Date(newDate);
          if (!isNaN(date.getTime())) {
            const lastDay = addDays(date, 60);
            handleChange('last_working_day', format(lastDay, 'yyyy-MM-dd'));
            if (!formData.notice_period || formData.notice_period === '') {
              handleChange('notice_period', '60 days');
            }
          }
        }
      }}
  ...
  const NoticePeriodField = ({ field, formData, handleChange }: any) => {
      ...
      <div className="flex items-center space-x-2">
        <div className="relative flex-1">
          <Input
            ...
            placeholder={t('e.g. 60 days')}
            className={!isEditable ? 'bg-muted pr-10' : 'pr-10'}
          />
          {!isEditable && (
        ...
        <Button
          ...
          title={isEditable ? t('Lock Field') : t('Edit Notice Period')}
        >
          {isEditable ? <Lock className="h-4 w-4" /> : <Pencil className="h-4 w-4" />}
        </Button>
      </div>
    );
  };
        ...
        formConfig={{
          fields: [
            ...
            { 
              ...
              type: 'custom', 
              required: true,
              render: (field, formData, handleChange) => <ResignationDateField field={field} formData={formData} handleChange={handleChange} />
            },
            { 
            ...
            { 
              ...
              label: t('Notice Period'), 
              type: 'custom',
              render: (field, formData, handleChange) => <NoticePeriodField field={field} formData={formData} handleChange={handleChange} />
            },
            { 
              name: 'reason', 
              label: t('Reason'), 
