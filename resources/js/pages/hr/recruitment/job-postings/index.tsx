...
export default function JobPostings() {
    ...
    switch (action) {
      ...
      case 'publish':
        if (!globalSettings?.is_demo) {
          toast.loading(t('Publishing Campaign - Job Posting...'));
        }
        router.put(route('hr.recruitment.job-postings.publish', item.id), {}, {
          ...
          onError: (errors) => {
            ...
            if (typeof errors === 'string') {
              ...
              toast.error(t('Failed to publish Campaign - Job Posting: {{errors}}', { errors: Object.values(errors).join(', ') }));
            }
          }
        });
        break;
      case 'unpublish':
        if (!globalSettings?.is_demo) {
          toast.loading(t('Unpublishing Campaign - Job Posting...'));
        }
        router.put(route('hr.recruitment.job-postings.unpublish', item.id), {}, {
          ...
          onError: (errors) => {
            ...
            if (typeof errors === 'string') {
              ...
              toast.error(t('Failed to unpublish Campaign - Job Posting: {{errors}}', { errors: Object.values(errors).join(', ') }));
            }
          }
        });
        break;
  ...
  const handleDeleteConfirm = () => {
    if (!globalSettings?.is_demo) {
      toast.loading(t('Deleting Campaign - Job Posting...'));
    }
    ...
    router.delete(route('hr.recruitment.job-postings.destroy', currentItem.id), {
      ...
      onError: (errors) => {
        ...
        if (typeof errors === 'string') {
          ...
          toast.error(t('Failed to delete Campaign - Job Posting: {{errors}}', { errors: Object.values(errors).join(', ') }));
        }
      }
    });
  };
  ...
  if (hasPermission(permissions, 'create-job-postings')) {
    pageActions.push({
      label: t('Add Campaign - Job Posting'),
  ...
  const columns = [
    { 
      ...
      label: t('Code'), 
      sortable: true,
      render: (value: any) => <div className="font-mono text-sm">{value}</div>
    },
    { 
      key: 'title', 
      label: t('Title'), 
    ...
    { 
      key: 'job_type.name', 
      label: t('Type'),
      render: (_: any, row: any) => row.job_type?.name || '-'
    },
    { 
      key: 'location.name', 
      label: t('Location'),
      render: (_: any, row: any) => row.location?.name || '-'
    },
    { 
      key: 'min_salary', 
    ...
    { 
      ...
      render: (value: any) => <div className="text-center">{value || 0}</div>
    },
    { 
      key: 'is_published', 
      label: t('Published'),
  ...
  const actions = [
    ...
    { 
      ...
      condition: (item: any) => !item.is_published
    },
    { 
      label: t('Unpublish'), 
      icon: 'Download', 
  ...
  return (
    <PageTemplate 
      title={t("Campaign - Job Postings")} 
      description={t("Manage job posting campaigns")}
    ...
    >
      ...
      <div className="bg-white dark:bg-gray-900 rounded-lg shadow overflow-hidden">
        ...
        <Pagination
          ...
          entityName={t("Campaign - Job Postings")}
      ...
      <CrudDeleteModal
        ...
        entityName="Campaign - Job Posting"
      />
    </PageTemplate>
  );
}