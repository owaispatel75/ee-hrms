      ...
      <Table>
        <TableHeader>
          <TableRow className="bg-gray-50/50 dark:bg-gray-900/50 border-b border-gray-100 dark:border-gray-800">
            <TableHead className="w-12 py-4 pl-6 text-xs font-semibold text-gray-500 uppercase tracking-wider">#</TableHead>
            {columns.map((column) => (
              <TableHead
                key={column.key}
                className={cn(
                  "py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider",
                  column.sortable && "cursor-pointer select-none group hover:text-gray-700 dark:hover:text-gray-300",
                  column.className
                )}
            ...
            {showActions && hasAnyActionPermission && <TableHead className="w-24 py-4 pr-6 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider">{t('Actions')}</TableHead>}
          </TableRow>
        </TableHeader>
        <TableBody>
          {data.length > 0 ? (
            data.map((row, index) => (
              <TableRow key={row.id || index} className="group hover:bg-gray-50 dark:hover:bg-gray-800/50 border-b border-gray-100 dark:border-gray-800 transition-colors duration-200">
                <TableCell className="font-medium py-4 pl-6 text-gray-500 text-sm">{from + index}</TableCell>
                {columns.map((col) => (
                  <TableCell
                    key={col.key}
                    className={cn(
            ...
            <TableRow>
              ...
              >
                  <div className="flex flex-col items-center justify-center text-muted-foreground p-8">
                     ...
                     <p className="text-lg font-medium text-gray-900 dark:text-gray-100">{t("No users found")}</p>
                     <p className="text-sm text-gray-500 max-w-sm mt-1">{t("Try adjusting your search or filters to find what you're looking for.")}</p>
                  </div>
              </TableCell>
            </TableRow>
          )}
