...
class ResignationController extends Controller
{
    ...
    public function update(Request $request, Resignation $resignation)
    {
        ...
        if ($request->status === 'completed') {
            ...
            if ($employee) {
                $employee->update(['employee_status' => 'terminated', 'date_of_exit' => $resignation->last_working_day]);
            }
            $user = User::find($resignation->employee_id);
            if ($user && $user->type === 'employee') {
                $exitTime = \Carbon\Carbon::parse($resignation->last_working_day)->setTime(18, 0, 0);
