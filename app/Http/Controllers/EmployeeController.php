...
class EmployeeController extends Controller
{
        ...
        if (Auth::user()->can('create-employees')) {
            try {
                ...
                $validator = Validator::make($request->all(), [
                    ...
                    'password' => $request->has('candidate_id') ? 'nullable|string|min:8' : 'required|string|min:8',
                    ...
                    'adhar_no' => 'nullable|string|max:50',
                ...
                $employee->employee_id = $request->employee_id ?: Employee::generateEmployeeId();
                $employee->biometric_emp_id = $employee->employee_id;
    ...
    public function show(Employee $employee)
    {
        if (Auth::user()->can('view-employees')) {
            ...
            // Load user with employee relationships and history
        $user = User::with([
            'employee.branch', 
            'employee.department', 
    ...
    public function update(Request $request, Employee $employee)
    {
        if (Auth::user()->can('edit-employees')) {
            ...
            try {
                ...
                $employee->process = $request->process;
                $employee->date_of_exit = $request->date_of_exit;
    ...
    public function storeDocument(Request $request, Employee $employee)
    {
        if (Auth::user()->can('edit-employees')) {
            ...
            try {
                ...
                $filePath = $file->storeAs('employee_documents/' . $employee->user_id, $fileName, 'public');
                ...
                return redirect()->back()->with('error', __('Failed to upload document: :message', ['message' => $e->getMessage()]));
            }
        }
        return redirect()->back()->with('error', __('Permission Denied.'));
    }
