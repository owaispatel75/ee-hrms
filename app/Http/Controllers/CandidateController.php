...
class CandidateController extends Controller
{
    ...
    public function store(Request $request)
    {
        $validator = Validator::make($request->all(), [
            ...
            'source_id' => 'nullable|exists:candidate_sources,id',
            'first_name' => 'required|string|max:255',
            'last_name' => 'required|string|max:255',
            'email' => 'required|email|max:255',
            'phone' => 'required|string|max:20',
            'status' => 'required|string|max:255',
            'sourced_by' => 'nullable|string|max:255',
    ...
    public function show(Candidate $candidate)
    {
        ...
        $offerTemplates = OfferTemplate::where('status', 'active')
            ->whereIn('created_by', getCompanyAndUsersId())
            ->get(['id', 'name']);
    ...
    public function updateStatus(Request $request, Candidate $candidate)
    {
        ...
        $validator = Validator::make($request->all(), [
            'status' => 'required|in:New,Screening,Interview,Offer,Offered,Hired,Rejected',
        ]);
    ...
    public function sendOffer(Request $request, Candidate $candidate)
    {
        if (!in_array($candidate->created_by, getCompanyAndUsersId())) {
            return redirect()->back()->with('error', 'You do not have permission to send offer to this candidate');
        }

        if (!in_array($candidate->status, ['Offer', 'Offered'])) {
            return redirect()->back()->with('error', 'Candidate status must be Offer or Offered to send an offer letter.');
        }
        ...
        try {
            $template = OfferTemplate::findOrFail($request->offer_template_id);
            
            // Generate variables dynamically from target candidate data mappings.
            $variables = [
                'candidate_name' => $candidate->first_name . ' ' . $candidate->last_name,
                'salary' => $request->salary,
                'start_date' => $request->start_date,
                'expiration_date' => $request->expiration_date,
                'job_title' => $candidate->job ? $candidate->job->title : 'N/A',
                'company_name' => Auth::user()->name ?? 'Company',
            ...
            $html = '<div style="font-family: Arial, sans-serif; line-height: 1.6; padding: 20px;">' . nl2br($content) . '</div>';
            $pdfContent = Pdf::loadHTML($html)->output();

            $attachmentName = 'Offer_Letter_' . $candidate->first_name . '_' . $candidate->last_name . '.pdf';

            // Send via Mailable. Let's make sure candidate email exists
            if (!$candidate->email) {
                return redirect()->back()->with('error', 'Candidate does not have an email address.');
            }

            Mail::to($candidate->email)->send(new CandidateOfferMail($candidate, $pdfContent, $attachmentName, $request->message));

            // Automatically create a record in the offers table.
            Offer::create([
                'candidate_id' => $candidate->id,
                'job_id' => $candidate->job_id,
                'created_by' => creatorId(),
                'offer_date' => now(),
                'position' => $candidate->job ? $candidate->job->title : 'General Application',
            ...
            return redirect()->back()->with('success', __('Offer letter sent successfully'));
            
        } catch (\Exception $e) {
            \Log::error('Offer Letter sending failed: ' . $e->getMessage());
            return redirect()->back()->with('error', __('Failed to send offer letter: :message', ['message' => $e->getMessage()]));
        }
    }
