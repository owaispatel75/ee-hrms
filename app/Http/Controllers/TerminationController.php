...
class TerminationController extends Controller
{
    ...
    public function update(Request $request, Termination $termination)
    {
        ...
        // SYNC STATUS: If status is completed, update employee status and user status
        if ($request->status === 'completed') {
            $employee = Employee::where('user_id', $termination->employee_id)->first();
            if ($employee) {
                $employee->update(['employee_status' => 'terminated', 'date_of_exit' => $termination->termination_date]);
            }
            $user = User::find($termination->employee_id);
            if ($user && $user->type === 'employee') {
                $exitTime = \Carbon\Carbon::parse($termination->termination_date)->setTime(18, 0, 0);
